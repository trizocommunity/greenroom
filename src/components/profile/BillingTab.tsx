      ...
      <Card>
        ...
        <CardContent>
          ...
          {status === 'ACTIVE' && payment && (
            <ActiveState payment={payment} onCreateFestival={() => router.push('/profile?tab=festivals')} />
          )}
          {status === 'EXPIRED' && (
...
function NotPaidState({ onPay, isPending }: { onPay: () => void; isPending: boolean }) {
  return (
    <div className="text-center py-6">
      <div className="w-16 h-16 rounded-full bg-amber-100 flex items-center justify-center mx-auto mb-4">
        <CreditCard className="h-8 w-8 text-amber-600" />
      </div>
      <h3 className="text-lg font-semibold mb-2">Complete Payment to Create a Festival</h3>
      ...
      <div className="bg-muted/50 rounded-lg p-4 mb-6 max-w-xs mx-auto">
        ...
        <div className="text-sm text-muted-foreground">One-time payment · 30 days validity</div>
      </div>

      <Button onClick={onPay} disabled={isPending} size="lg" className="h-12 px-8">
        {isPending ? (
          <>
  ...
  return (
    <div className="text-center py-6">
      ...
      <p className="text-muted-foreground mb-6">
        Your festival pass is active. Create your festival now.
      </p>
      ...
      <div className="bg-muted/50 rounded-lg p-4 mb-6 max-w-sm mx-auto">
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <div className="font-medium">{format(validFrom, 'dd MMM yyyy')}</div>
          </div>
          <div>
            <div className="text-muted-foreground">Valid Until</div>
            <div className="font-medium text-green-600">{format(validUntil, 'dd MMM yyyy')}</div>
          </div>
        </div>
      </div>

      <Button onClick={onCreateFestival} size="lg" className="h-12 px-8">
...
function ExpiredState({ onRenew, isPending }: { onRenew: () => void; isPending: boolean }) {
  return (
    <div className="text-center py-6">
      <div className="w-16 h-16 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-4">
        <AlertCircle className="h-8 w-8 text-red-600" />
      </div>
      <h3 className="text-lg font-semibold mb-2">Your Festival Pass Has Expired</h3>
      <p className="text-muted-foreground mb-6 max-w-md mx-auto">
        Renew your pass to create new festivals. Your existing festivals remain accessible in read-only mode.
      </p>

      {/* Price Display */}
      <div className="bg-muted/50 rounded-lg p-4 mb-6 max-w-xs mx-auto">
        <div className="text-3xl font-bold text-foreground">₹1000</div>
        <div className="text-sm text-muted-foreground">Renew for another 30 days</div>
      </div>

      <Button onClick={onRenew} disabled={isPending} size="lg" className="h-12 px-8">
        {isPending ? (
          <>
            <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
            Processing...
