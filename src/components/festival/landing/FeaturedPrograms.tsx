      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="flex justify-between items-end mb-12">
            <div>
                ...
                <p className="text-muted-foreground">Don't miss the key events happening now.</p>
            </div>
            <Link href={`/festival/${slug}/sessions`} className="hidden md:block">
                <Button variant="ghost" className="group">
                    View All Programs <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </Button>
            </Link>
        </div>
            ...
            {featured.map((session, i) => (
                ...
                >
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <Badge className={
                            session.status === 'LIVE' ? "bg-red-500 hover:bg-red-600 animate-pulse" : 
                    ...
                    <h3 className="text-xl font-semibold mb-3 group-hover:text-(--accent) transition-colors" style={{'--accent': accentColor} as any}>
                        {session.title}
                    </h3>
