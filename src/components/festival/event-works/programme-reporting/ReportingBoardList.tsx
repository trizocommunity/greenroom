...
export function ReportingBoardList({
  ...
  const statusTone = (status: string) => {
    ...
    if (status === "CLOSED" || status === "COMPLETED") return "border-purple/30 bg-purple/10 text-purple";
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  {(() => {
                    if (item.reportingSession?.status !== "CLOSED" && item.reportingSession?.status !== "COMPLETED") return null;
