...
export function IndividualAssignmentDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Import action dynamically or assume it's imported at top (we will add import)
      const { createAssignmentAction } = await import(
        "@/server/actions/assignment.actions"
      );

      await Promise.all(
        selectedProgrammeIds.map((progId) =>
          createAssignmentAction(festivalId, {
            programmeId: progId,
      ...
      toast.success(
        `Successfully assigned to ${selectedProgrammeIds.length} programmes`,
      );
      setOpen(false);
    } catch (error) {
      console.error(error);
      ...
      <DialogContent className="max-w-md flex flex-col max-h-[85vh]">
        ...
        >
              ...
              <SelectContent>
                {filteredStudents.map((s: any) => (
                  <SelectItem key={s.id} value={s.id}>
                    {s.name} ({s.registrationNumber || "No Reg"}) -{" "}
                    {s.group?.name}
                  </SelectItem>
                ))}
            ...
            <div className="flex items-center justify-between">
              ...
              {selectedProgrammeIds.length > 0 && (
                <Badge variant="secondary">
                  {selectedProgrammeIds.length} selected
                </Badge>
              )}
            </div>
            ...
            <div className="border rounded-md flex-1 overflow-hidden relative bg-muted/10">
              {!formData.studentId ? (
                <div className=" flex items-center justify-center text-sm text-muted-foreground p-4 text-center">
                  {formData.categoryId
                    ? "Select a student to view available programmes."
                    : "Select a category to view available students."}
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {alreadyAssigned ? (
                            <Badge
                              variant="outline"
                              className="text-[10px] h-5 bg-muted"
                            >
                              Assigned
                            </Badge>
                          ) : (
          ...
          <DialogFooter>
            ...
            <Button
              type="submit"
              disabled={isSubmitting || selectedProgrammeIds.length === 0}
            >
              {isSubmitting && (
