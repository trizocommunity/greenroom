...
/**
 * `isCustomDomain` comes from the proxy header, because on a branded host the
 * festival is served at `/` and `usePathname()` never carries the slug.
 */
export function FestivalNotFoundView({
  isCustomDomain,
}: {
  isCustomDomain: boolean;
}) {
  const pathname = usePathname();
  // App host: the first URL segment is the festival slug. The layout only calls
  // notFound() after findFestivalBySlug succeeds, so this is always a real (or
  // formerly-real) festival. If it's missing, fall back to site root.
  const segments = (pathname ?? "").split("/").filter(Boolean);
  const festivalSlug = isCustomDomain ? undefined : segments[0];
