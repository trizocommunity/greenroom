...
import { Check, Loader2, MoreHorizontal, Shield, Trash2, X } from "lucide-react"
...
import { useUsers, User, useUpdateUser, useDeleteUser } from "@/hooks/useUsers"
...
export function UsersTable() {
  ...
  const columns: ColumnDef<User>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
            <Badge variant={role === "SUPER_ADMIN" ? "default" : "secondary"} className="font-mono text-xs">
                {role}
            </Badge>
        )
      },
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
             ...
             <span className="capitalize text-sm text-muted-foreground">{isActive ? "Active" : "Inactive"}</span>
          </div>
        )
      },
    },
  ...
  const isActionLoading = updateUserMutation.isPending || deleteUserMutation.isPending
  ...
  if (isError) {
    return <div className="py-10 text-center text-red-500">Failed to load users.</div>
  }
