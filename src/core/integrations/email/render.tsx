...
import { FestivalInvitationEmail, festivalInvitationSubject } from "./kinds/festival-invitation";
import { MagicLinkEmail } from "./kinds/magic-link";
import { TeamLeaderOtpEmail, teamLeaderOtpSubject } from "./kinds/team-leader-otp";
...
export function resolveTheme(kind: EmailKind, override?: EmailTheme): EmailTheme {
		...
		case "magic_link": {
			element = (
				<MagicLinkEmail
					...
					expiresInMinutes={kind.expiresInMinutes ?? DEFAULT_MAGIC_LINK_EXPIRY_MINUTES}
		...
		case "festival_invitation": {
			element = (
				<FestivalInvitationEmail
					...
					role={kind.role}
					expiresInHours={kind.expiresInHours ?? DEFAULT_INVITATION_EXPIRY_HOURS}
					theme={effectiveTheme}
				/>
			);
