	...
	return (
		...
		>
			...
			<Text className="m-0 mb-6 font-sans text-15 text-fg-2">
				The{" "}
				<span className="text-fg font-semibold">{festivalName}</span> festival
				access window ends in{" "}
				<span className="text-fg font-semibold">
					{daysRemaining} day{daysRemaining === 1 ? "" : "s"}
				</span>{" "}
				(on{" "}
				<span className="text-fg font-semibold">{expiresOn}</span>). After
				that, the dashboard and public pages go read-only, and data is
				archived. Export anything you still need before the window closes.
			</Text>
