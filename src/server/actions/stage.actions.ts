...
import { stage, festival as festivalTable, user } from "@/server/db/schema";
...
export async function createStage(festivalId: string, data: StageData) {
  ...
  const createdBy =
    userRecord?.displayName || userRecord?.fullName || userRecord?.email || "Unknown";
  ...
  if (festivalRecord) {
    revalidatePath(`/dashboard/${festivalRecord.slug}/event-works/stage-management`);
  }
}
...
export async function updateStage(stageId: string, data: StageData) {
  ...
  await db.update(stage).set({ name: data.name, description: data.description }).where(eq(stage.id, stageId));
...
export async function deleteStage(stageId: string) {
  ...
  await UsageCounterService.incrementUsage(stageRecord.festivalId, "stages", -1);
