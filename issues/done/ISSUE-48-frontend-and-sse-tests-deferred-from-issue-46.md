...
- [ ] Tests run under `pnpm test:integration` and finish in <60s combined.
...
- [ ] `pnpm test:integration` covers all 9 channels end-to-end.
- [ ] `pnpm lint`, `pnpm check`, `pnpm test` green.

## Verification

```bash
pnpm test:integration -- sse-channels sse-auth
```
