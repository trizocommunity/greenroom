...
- **Drop-in schema additions** via `pnpm dlx @better-auth/cli generate`. We keep our `user` table — Better Auth just adds three columns and three new tables.
...
**Tasks**
1. `pnpm add better-auth nanostores`
...
4. Run `pnpm dlx @better-auth/cli generate`; **hand-review** the generated SQL diff
...
**Verification**
- `pnpm lint` clean
- `pnpm test:unit` clean (existing tests still pass)
...
12. Delete `jose` from `package.json` (`pnpm remove jose`)
...
- `pnpm lint` clean
- `pnpm test:unit` clean
- `pnpm test:integration` covers every route handler from the inventory below
...
2. Run `pnpm dlx @better-auth/cli generate`; apply migration (`twoFactor` table)
