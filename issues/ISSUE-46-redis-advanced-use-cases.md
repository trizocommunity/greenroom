...
- **Local dev**: `npx inngest-cli@latest dev` runs alongside `next dev`. Vercel + Inngest deploy integration auto-configures the prod webhook.
...
| 3 | Function co-location | Same process as export function; separate event names | One `npm run worker` equivalent (Inngest handles routing). |
...
- [ ] `npm run lint`, `npm run check`, `npm test`, `npm run test:integration` green.
