...
| 4 | `db:setup` flow | `npm run db:start && npm run db:push && npm run db:seed` now starts Redis too | Single command brings the whole stack up. |
...
```bash
docker compose up -d
npm run test:integration -- redis-cache-seam
npm run dev
