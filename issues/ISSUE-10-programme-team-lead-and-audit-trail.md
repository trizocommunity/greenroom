...
| 2 | Seed data | Modify existing `scripts/seed/programmes.ts` — no new seed file |
| 3 | PRO-only enforcement | Both backend and frontend |
| 4 | Tests | None for this iteration — manual QA only |
| 5 | Lead storage shape | New `programme_team_lead` table (DB-enforced uniqueness) |
| 6 | Is a lead required? | Mandatory — save blocked if missing |
| 7 | Team leader forms a team they're not on? | Yes — no link between portal operator and team membership |
| 8 | Lead removed from a team? | Block the delete until a replacement is appointed |
| 9 | `programme_team_lead` rows for INDIVIDUAL? | No — only GROUP |
| 10 | Existing `student.isTeamLeader`? | Untouched |
...
1. **No concept of programme-level team lead.** Festival admins and team-leaders can create GROUP teams, but there's no record of which student is the team's contact person, code-letter recipient primary, or notification target. Everything currently keys off the code letter, which is shared across the team.
...
- Existing `student.isTeamLeader` — untouched.
- Existing team-leader OTP/login flow (`src/core/auth/participant-guard.ts`, `participant-session.ts`) — untouched.
- Changing the GROUP result-storage model (separate planned issue).
- Audit log retention/pruning — none.
...
**Add to `src/core/database/schema.ts`** (Track A — picked up by `db:push`):
- New table `programme_team_lead` with columns `id`, `programmeId` (FK→programme, cascade), `groupId` (FK→group, cascade), `teamNumber`, `studentId` (FK→student, cascade), `appointedBy`, `appointedByRole` (`"ADMIN" | "TEAM_LEADER"`), `appointedByName`, `appointedByEmail`, `appointedAt`, `createdAt`, `updatedAt`. Constraints: `UNIQUE (programmeId, groupId, teamNumber)`, indexes on `programmeId` and `studentId`.
...
**Author matching SQL file** (Track B — production history):
...
Frontend consumes via `useFeature("programme.teamLead")` and `useFeature("programme.auditDrawer")`. Server actions throw `AppError("FEATURE_NOT_AVAILABLE")` when `!isProTier(festival.tier)` — checked in every new server action AND every new feature-gated write path.

### 3. New service — `src/features/programme-team-leads/services/programme-team-lead.service.ts`

| Function | Behaviour | Audit log |
|---|---|---|
| `appointTeamLead(input)` | INSERT row; refuses if `programme.type !== "GROUP"`; refuses if student not in team; refuses if a lead row already exists for the team (UI catches this earlier) | logs `APPOINT_TEAM_LEAD` |
| `replaceTeamLead(input)` | UPDATE same row's `studentId` + `appointedBy*` + `appointedAt` | logs `REPLACE_TEAM_LEAD` |
...
| `getTeamLeadForTeam({ programmeId, groupId, teamNumber })` | lookup | — |
| `getProgrammeTeamLeads(programmeId)` | grouped by `(groupId, teamNumber)` | — |

### 4. Assignment integration

`src/features/assignments/services/assignment.service.ts::assignStudentsToProgramme` accepts a new input shape (GROUP only):
```ts
teamLeadsByTeam?: Record<`${groupId}:${teamNumber}`, studentId>
```

Validation, all inside the same transaction as the assignments INSERT:
- If programme is GROUP and any team has no lead picked → throw `AppError("EACH_TEAM_MUST_HAVE_LEAD")`.
- If programme is GROUP and a picked lead is not in the team → throw `AppError("LEAD_NOT_IN_TEAM")`.
- INSERT each `programme_team_lead` row.
- Logs `ASSIGN_STUDENTS` + `APPOINT_TEAM_LEAD` per team.
...
- If programme is GROUP and the assignment's `(groupId, teamNumber)` matches a `programme_team_lead` row:
  - If the deleted student IS the lead and the team has only one remaining member → return `{ ok: false, reason: "TEAM_WOULD_BE_EMPTY" }`.
  - If the deleted student IS the lead and the team has other members → require caller to pass `replacementLeadStudentId`; if missing, return `{ ok: false, reason: "LEAD_MUST_BE_REPLACED", teamContext }`.
...
"ASSIGN_STUDENTS"  | "REMOVE_ASSIGNMENT"
...
| `src/features/assignments/services/assignment.service.ts` | ASSIGN_STUDENTS, REMOVE_ASSIGNMENT |
...
The existing `/super-admin/audit-logs` page already renders every entry in a flat table — no UI work needed there.

### 6. Programme detail loader — `src/features/programmes/loaders/programme-detail.loader.ts`
...
getProgrammeDetailForDrawer(programmeId): {
  ...
  teamLeads:        { [groupId]: { [teamNumber]: { studentId, studentName, chestNumber } } },
  auditTimeline:    Array<{ at, action, actorName, actorEmail, targetType, targetId }>,
}
```

Implemented as parallel `Promise.all` of indexed queries — no transaction needed (the loader is read-only).

### 7. Programme drawer UI

Three panels appended to the existing drawer at `AssignmentsClient.tsx:754-1083`:

- **Panel A — Summary (always visible).** Reporting started/closed (with names), reported count; judging started/completed, scored count; results saved/published/announced (with names).
- **Panel B — Programme Team Leads** (`useFeature("programme.teamLead") && programme.type === "GROUP"`). Per team: members list with lead badge (`Alice (#123) · LEAD`), code letters assigned.
- **Panel C — Audit Timeline** (`useFeature("programme.auditDrawer")`). Reverse-chronological audit-log entries scoped to this programme (action label, actor name + email, timestamp).

### 8. Frontend touchpoints (lead picker)

- `src/components/student/team-leader/AssignProgrammesClient.tsx` — lead picker inline below each team box; save disabled until selection made.
- `src/components/festival/pre-event-works/assignments/AssignmentsClient.tsx` — same picker in admin team-creation flow; lead badge in the team row.
...
|---|---|---|---|
| 1 | **DB reset seed update** | Modify `scripts/seed/programmes.ts` GROUP branch to insert one `programme_team_lead` per team. Run `pnpm db:reset`. | `psql \d programme_team_lead`; seed summary lists ≥1 team leads |
...
| 5 | **Programme team lead — backend** | `programme-team-lead.service.ts` + actions; integration into `assignStudentsToProgramme`; delete-block in repository. | Manual: assign team without lead → error; with lead → success |
| 6 | **Programme team lead — frontend** | Lead picker in `AssignProgrammesClient.tsx` and `AssignmentsClient.tsx`; lead badge in drawer. | Manual: form behaviour |
...
### Modified
- `scripts/seed/programmes.ts` — insert `programme_team_lead` rows in GROUP branch
- `src/core/database/schema.ts` — add `programme_team_lead` + audit columns on `programme`/`result`
- `src/config/pricing.ts` — two new flags
- `src/features/plan-features/services/features-tags.ts` — two new tags
- `src/features/auth/services/audit-log.service.ts` — new enum values
- `src/features/assignments/services/assignment.service.ts` — accept `teamLeadsByTeam`, transactional lead insert, delete-block, audit call
- `src/features/assignments/repositories/assignment.repository.ts` — delete-block, audit call
- `src/components/student/team-leader/AssignProgrammesClient.tsx` — lead picker
- `src/components/festival/pre-event-works/assignments/AssignmentsClient.tsx` — admin lead picker + 3 drawer panels
- 9 backend files listed in §"Audit log vocabulary" (audit-log wiring only)
...
| `deleteAssignment` cascade surprises team leads | Pre-check returns structured error before SQL delete — no constraint-violation leaks |
...
| Feature flag changes break existing flows | New flags default to `false` everywhere except PRO — safe rollout |
...
1. `pnpm db:reset` produces expected seed (1 PRO festival, 3 groups × 5 students, 2 leaders each, GROUP programmes with leads).
2. PRO festival: assign a new GROUP programme with 2 teams — both save only after picking a lead.
3. PRO festival: try to delete the lead from a 1-member team — error appears, deletion refused.
4. PRO festival: replace the lead on a multi-member team — succeeds, drawer shows the new lead.
...
10. `student.isTeamLeader` and the team-leader OTP portal still work exactly as before.
...
- Existing team-leader portal: `src/components/student/team-leader/AssignProgrammesClient.tsx`
