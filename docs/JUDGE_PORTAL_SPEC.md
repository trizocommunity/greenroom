# Judge Portal — New Judgement Method (Design Spec)

> **Status:** Draft — design only, no implementation yet
> **Version:** 0.1
> **Last Updated:** July 2026
> **Replaces:** External judgement flow (link + PIN) described in `judgement.actions.ts`. Legacy BASIC-tier marks/timer flow is untouched by this doc.
...
- Admin manually creates a `judgement_config`, generates a token + PIN per programme.
...
2. **Stage Manager starts a programme** — Opens "Start Judgement" on a programme. A dialog opens to search and select judge(s) for this specific programme, or quick-create a judge on the spot if they're not in the system yet (e.g. a substitute). Dialog pre-fills with the last-used judges for that stage to avoid re-picking every time.
...
### Start Judgement dialog (Stage Manager-facing)
...
```
not_started ──(Start Judgement + pick judges)──> live ──(submit)──> submitted ──(publish)──> published
     ^                                                                  │
...
- **Stage-level pool** (loose) — the general list of judges who usually work a stage. Used only to pre-fill the Start Judgement dialog; not an enforced restriction.
- **Programme-level assignment** (binding) — decided at Start Judgement time, per programme. This is what actually controls who can see and score that live programme.
...
| Today | New method |
|---|---|
| `judgement_link` (token + PIN + expiry per programme) | Stage-owned portal credential, created once per stage |
...
Untouched: scoring panel UI, `judgement_score` table, scoring-policy/grading logic, results/leaderboard pipeline.
...
- Should the quick-created judge (from the Start Judgement dialog) be scoped to that stage only until an Admin completes their profile, or immediately available festival-wide?
