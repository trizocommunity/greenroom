...
- **Sidebar:** `FestivalDashboardSidebar` uses `useFeatures()` and hides items by feature (e.g. Settings, Members, Stage Management, Schedule, Sessions, QR Codes, Media, News, Analytics). BASIC Event Works: Scoring Policy, Scoring (`/event-works/marks`), Leaderboard when `liveScoreboard` is enabled.
